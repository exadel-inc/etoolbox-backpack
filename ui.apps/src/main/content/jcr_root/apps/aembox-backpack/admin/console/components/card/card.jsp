...
<coral-quickactions target="_prev" alignmy="left top" alignat="left top">
    <coral-quickactions-item icon="dataRefresh" class="foundation-collection-action" data-foundation-collection-action='{"action": "cq.wcm.open", "data": {"cookiePath":"<%= request.getContextPath() %>/","href":"/tools/aembox/backpack/buildPackage.html?path=<%= xssAPI.getValidHref(path) %>"}}'><%= xssAPI.encodeForHTML(i18n.get("Build")) %></coral-quickactions-item><%

    if (resource != null && hasPermission(acm, resource, Privilege.JCR_READ)) {
%>
    <coral-quickactions-item icon="edit" class="foundation-collection-action" data-foundation-collection-action='{"action":"foundation.dialog","data":{"nesting":"hide","src":"/mnt/overlay/aembox-backpack/admin/console/page/content/editpackagedialog.html?packagePath=<%= xssAPI.getValidHref(path) %>"}}'
